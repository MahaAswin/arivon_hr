...
const RecruiterSidebar = ({ isOpen, toggleSidebar }) => {
  const { recruiter, recruiterLogout } = useRecruiter();
    ...
    <>
      ...
      `}>
        <div className="p-8 flex items-center justify-between">
          <Link to="/recruiter/dashboard" className="flex items-center gap-0 group">
            <img src={logo} alt="Arivon" className="w-12 h-12 object-contain group-hover:scale-110 transition-transform" />
            <span className="text-2xl font-black tracking-tighter text-white">RIVON<span className="text-primary text-xs ml-1 font-bold">PRO</span></span>
          </Link>
        ...
        <div className="absolute bottom-0 left-0 w-full p-6 space-y-4">
          ...
          <div className="text-center">
            <p className="text-[10px] font-black uppercase tracking-[0.3em] text-white/20">Arivon for Business v1.0</p>
          </div>
        </div>
      </aside>
    </>
